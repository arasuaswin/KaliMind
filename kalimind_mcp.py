...
DEFAULT_KALIMIND_SERVER = "http://localhost:5000" # change to your linux IP
...
class KaliToolsClient:
    ...
    def __init__(self, server_url: str, timeout: int = DEFAULT_REQUEST_TIMEOUT):
        """
        Initialize the KaliMind Client
        
        Args:
            server_url: URL of the KaliMind API Server
            timeout: Request timeout in seconds
        """
        self.server_url = server_url.rstrip("/")
        self.timeout = timeout
        logger.info(f"Initialized KaliMind Client connecting to {server_url}")
    ...
    def check_health(self) -> Dict[str, Any]:
        """
        Check the health of the KaliMind API Server
...
def setup_mcp_server(kalimind_client: KaliToolsClient) -> FastMCP:
    """
...
def parse_args():
    ...
    parser.add_argument("--server", type=str, default=DEFAULT_KALIMIND_SERVER, 
                      help=f"KaliMind API server URL (default: {DEFAULT_KALIMIND_SERVER})")
...
def main():
    ...
    # Initialize the KaliMind client
    kalimind_client = KaliToolsClient(args.server, args.timeout)
    
    # Check server health and log the result
    health = kalimind_client.check_health()
    if "error" in health:
    ...
    mcp = setup_mcp_server(kalimind_client)
